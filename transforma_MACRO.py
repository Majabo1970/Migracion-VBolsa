...
def process_line(line, relationships):
    ...
    return {
        'InstructingParty': '33',
        'SettlementParty': '33',
        'SecuritiesAccount': '33/' + relationships.get(comitente_bma, 'Sin Cuenta'),
        'Instrument': line[20:25].strip().lstrip('0'),
        'InstrumentIdentifierType': 'LOCAL_CODE',
        'CSDOfCounterparty': 'CVSA',
        'SettlementCounterparty': '171',
        'SecuritiesAccountOfCounterparty': '171/' + comitente_bma,
        'InstructionReference': generate_random_string(16),
        'Instrument(MovementOfSecurities)': 'RECEIVE',
...
def main():
    ...
    output_file_macro1 = 'destino_MACRO.si1'
    output_file_efectivo = 'efectivo.si1'
    output_file_saldos = 'saldos_bloqueados.si1'
    ...
    with open(input_file, 'r') as infile:
        ...
        for line in lines:
            ...
            if instrument in ["06000", "07000", "08000", "09000", "10000", "20000"]:
                efectivo.append(data)
            elif posicion == "02":
                saldos_bloqueados.append(data)
            else:
                destino_macro1.append(data)

    with open(output_file_efectivo, 'w') as outfile:
        outfile.write(header)
        for data in efectivo:
            outfile.write(';'.join(data.values()) + '\n')

    with open(output_file_saldos, 'w') as outfile:
        outfile.write(header)
        for data in saldos_bloqueados:
            outfile.write(';'.join(data.values()) + '\n')
